...
& $installer -AgentDir $agent -CodexHome $codex -RiggingSkillsRoot ''
...
$refused = $false
try { & $installer -AgentDir $agent -CodexHome $codex -RiggingSkillsRoot '' } catch { $refused = $true }
if (-not $refused -or (Get-FileHash "$agent/AGENTS.md").Hash -ne $edited) { throw 'Local edit not protected' }
& $installer -AgentDir $agent -CodexHome $codex -RiggingSkillsRoot '' -BackupConflicts
...
$refused = $false
try { & $installer -AgentDir $conflictHome -CodexHome $codex -RiggingSkillsRoot '' } catch { $refused = $true }
